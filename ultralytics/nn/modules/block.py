...
class BAW_Concat(nn.Module):
    def __init__(self, dimension=1):
        super(BAW_Concat, self).__init__()
        self.d = dimension
        self.w = nn.Parameter(torch.ones(2), requires_grad=True)  # 两个输入就用两个权重
        self.epsilon = 1e-4
    ...
    def forward(self, x):
        ...
        weight = weight / (weight.sum() + self.epsilon)  # 创建新张量
